...
jazz_corpus <-
  ...
  prep(songs_corpus |> mutate(track.name = str_trunc(str_c(category," - ", track.name), 30))) |>
  juice() |>
...
playlist_data_for_join <- full_dendo %>%
  ...
  mutate(label = str_trunc(str_c(category," - ", track.name), 30))

data_for_dendo$lables

data_for_dendo$labels$label <- factor(data_for_dendro$labels$label)
...
data_for_dendro |>
  ...
  geom_text(data = label(data_for_dendro), aes(x, y, 
                                   label=label, 
                                   hjust=0, 
                                   colour=full_dendo$category), size=3) +
  coord_flip() + 
